...
classdef weak_classifier_list
    ...
    methods
        ...
        function obj=weak_classifier_list()
        end
        ...
        %% add_organized_classifier
        % adds the classifier in an ordered way.
        %##################################################################
        %  IS BETTER NOT TO USE THIS UNTIL THE ADABOOST IS FINISHED
        %##################################################################
        function obj = add_organized_classifier(obj, hl_code, thres, e_rate)
            if obj.list_size == 0;
                obj = obj.weak_classifier_list_init(hl_code, thres, e_rate);
            elseif obj.list_size == 1;
                if obj.error_rate < e_rate
                    pos = 2;
                else
                    pos = 1;
                end
            else
                pos = bubble_search(e_rate);
            end
            obj = obj.add_classifier_i_pos(obj, hl_code, thres, e_rate, pos);
        end
        ...
        %% bubble_search
        % searchs for the closest e_rate value inside our list of
        % error_rates
        %##################################################################
        %  IS BETTER NOT TO USE THIS UNTIL THE ADABOOST IS FINISHED
        %##################################################################
        function res = bubble_search(e_rate)
            res = 0;
            if obj.is_in_range(pos)
                if obj.list_size == 1
                    % size is 0 or 1, so we know the result already
                    res = 1;
                elseif obj.list_size > 1;
                    res = bubble_search_rec(e_rate, 1, obj.list_size);
                    
                    % if all the objects are minor or major than our error
                    % then we should return the proper number
                    if res == obj.is_minor
                        res = 1;
                    elseif res == obj.is_major
                        res = obj.list_size;
                    end
                end
            end
        ...
        %  IS BETTER NOT TO USE THIS UNTIL THE ADABOOST IS FINISHED
        %##################################################################
%         function res = bubble_search_rec(e_rate, start_point, end_point)
%             if start_point == end_point
%                 % case simple 1: both points are the same, so this must be
%                 % the correct position (we have checked in the
%                 % initialization if the we had only one component in our
%                 % vectors, making sure that we do the recursivity).
%                 res = start_point;
%             elseif obj.error_rate(start_point) > e_rate
%                 % the point is not in our range and is minor than the start
%                 % point
%                 res = obj.is_minor;
%             elseif obj.error_rate(end_point) < e_rate
%                 % the point is not in our range and is major than the start
%                 % point
%                 res = obj.is_major;
%             else
%                 % the point value IS in our range
%                 % calcules the middle point of our range:
%                 my_middle = (start_point + end_point)/2;
%                 bubble_search_rec(e_rate, start_point, end_point)
%                 obj.error_rate(start_point) <= e_rate &&...
%                 obj.error_rate(start_point) >= e_rate
%                 % the error-position is inside this range
%                 if 
%                 end
%             
%                 
